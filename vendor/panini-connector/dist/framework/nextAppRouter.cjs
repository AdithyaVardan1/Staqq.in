...
// src/framework/nextAppRouter.ts
var nextAppRouter_exports = {};
__export(nextAppRouter_exports, {
  createHandler: () => createHandler
});
module.exports = __toCommonJS(nextAppRouter_exports);
...
function createHandler(opts) {
  ...
  return async function POST(request) {
    ...
    const ts = request.headers.get("x-compass-timestamp");
    const sig = request.headers.get("x-compass-signature");
    try {
      verify({ secret, timestamp: ts, signature: sig, rawBody, replayWindowSeconds: replayWindow });
    } catch (e) {
      if (e instanceof VerificationError) {
        return jsonResponse(401, { ok: false, error: "signature verification failed" });
      }
      throw e;
    }
    let envelope;
    try {
      ...
      if (e instanceof EnvelopeError) {
        return jsonResponse(400, { ok: false, error: `bad envelope: ${e.message}` });
      }
      throw e;
    }
    try {
      ...
      if (isNotFound(e)) {
        return jsonResponse(404, { ok: false, error: e.message });
      }
      console.error("[panini-connector] dispatch failed:", e);
      return jsonResponse(500, {
        ok: false,
        error: `internal: ${e.name || "Error"}`
      });
    }
  };
}
