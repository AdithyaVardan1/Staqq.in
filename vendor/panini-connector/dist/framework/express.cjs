...
function mount(app, opts) {
  const path = opts.path ?? "/api/seo-connector";
  ...
  const rawBodyMiddleware = (req, _res, next) => {
    if (req.body !== void 0 && typeof req.body === "object") {
      const err = new Error(
        "panini-connector: raw request body already consumed by JSON parser. Move express.json() AFTER mount(), or exclude the connector path from it."
      );
      return next(err);
    }
    const chunks = [];
  ...
  app.post(path, rawBodyMiddleware, handler);
}
